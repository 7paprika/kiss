...
const strategyGuides: Record<string, StrategyGuide> = {
  ...
  abc_trading: {
    core: "A 저점, B 반등 고점, C 눌림 저점이 만들어진 뒤 가격이 B를 다시 돌파하면 추세 재개로 보는 가격 행동 매매입니다.",
    logic: ["최근 구조에서 A 저점 이후 B 고점과 C 눌림을 확인합니다.", "C 저점이 A보다 높고 눌림 폭이 minPullbackPct 이상이면 유효한 ABC 구조로 봅니다.", "현재가가 B 고점을 breakoutPct 이상 돌파하면 BUY를 반환합니다.", "C 저점을 다시 이탈하면 패턴 실패로 보고 SELL 청산 신호를 반환합니다."],
    effective: ["하락 후 첫 반등과 눌림이 깔끔하게 나온 종목", "추세 전환 초입에서 전고점 돌파가 거래량과 함께 나올 때", "손절 기준을 C 저점 아래로 짧게 둘 수 있는 구간"],
    caution: ["C가 A보다 낮아지면 상승 ABC가 아니라 하락 지속일 수 있습니다.", "B 돌파 직후 윗꼬리가 길면 실패 돌파 위험이 큽니다.", "현재 시스템의 SELL은 공매도 진입이 아니라 보유분 청산 신호입니다."],
    tip: "ABC 매매는 C 저점 손절이 핵심입니다. 자동 청산 손절폭을 C 저점 기준보다 크게 벌리지 않는 편이 좋습니다.",
  },
  fractal_trading: {
    core: "좌우 봉보다 높은 프랙탈 고점과 낮은 프랙탈 저점을 찾아, 상단 프랙탈 돌파는 매수·하단 프랙탈 이탈은 청산으로 쓰는 구조 돌파 전략입니다.",
    logic: ["leftRightBars 기준으로 좌우 봉보다 높은 고점을 상단 프랙탈로 확정합니다.", "현재가가 확정된 상단 프랙탈을 breakoutPct 이상 돌파하면 BUY입니다.", "현재가가 확정된 하단 프랙탈을 이탈하면 SELL 청산 신호입니다.", "손절 기준은 최근 하단 프랙탈 또는 현재 저점에 stopBufferPct를 반영해 표시합니다."],
    effective: ["고점·저점 구조가 계단식으로 정리되는 추세 초입", "전고점 돌파가 반복되는 강한 종목", "짧은 봉 기준으로 분봉 스윙을 자동화할 때"],
    caution: ["프랙탈은 좌우 봉이 지나야 확정되므로 완전한 선행 신호가 아닙니다.", "횡보장에서는 상·하단 프랙탈 돌파가 잦은 속임수로 바뀔 수 있습니다.", "유동성이 낮은 종목은 단일 호가 고저점으로 프랙탈이 왜곡될 수 있습니다."],
    tip: "leftRightBars를 늘리면 신호는 줄지만 구조는 더 선명해집니다. 빠른 분봉은 2, 일봉 스윙은 3 이상을 검토해 보세요.",
  },
  channel_trading: {
    core: "이전 channelBars 구간의 최고가·최저가로 돈치안 채널을 만들고, 상단 돌파는 매수·하단 이탈은 청산으로 보는 돌파 매매입니다.",
    logic: ["현재 봉을 제외한 최근 channelBars개의 고가 최댓값을 상단 채널, 저가 최솟값을 하단 채널로 계산합니다.", "현재가가 상단 채널을 breakoutPct 이상 돌파하면 BUY입니다.", "현재가가 하단 채널을 이탈하면 SELL 청산 신호입니다.", "채널 폭과 손절 기준을 indicators에 노출해 좁은 박스 돌파와 넓은 변동성 돌파를 구분합니다."],
    effective: ["박스권 상단 돌파 후 추세가 이어지는 장", "거래량이 증가하며 이전 매물대를 넘어서는 종목", "명확한 고저 채널 안에서 압축 후 방향성이 나오는 구간"],
    caution: ["돌파 후 바로 채널 안으로 되돌아오면 실패 돌파 가능성이 큽니다.", "너무 짧은 channelBars는 잡음에 민감하고, 너무 긴 기간은 진입이 늦습니다.", "현재 시스템의 SELL은 하락 베팅이 아니라 기존 보유분 청산입니다."],
    tip: "채널 매매는 추세장에 강하고 횡보장에는 약합니다. 거래량 증가와 자동 트레일링 스탑을 함께 확인하는 방식이 좋습니다.",
  },
