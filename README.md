...
한국투자증권 KIS Open API 기반 개인용 자동매매 대시보드입니다.

## 개인용 운영 보안 설정

이 프로젝트는 단일 사용자 운영을 기본으로 합니다. 사용자별 데이터 분리는 유지하되, 다른 인증 사용자가 임의로 접속하지 못하도록 배포 환경에서는 다음 값을 반드시 설정하세요.

- `OWNER_OPEN_ID`: 접속을 허용할 본인 Manus/OAuth openId. 값이 설정되면 해당 openId가 아닌 모든 인증 사용자는 API와 Socket.IO 접속이 거부됩니다. `NODE_ENV=production`에서는 이 값이 없으면 접근을 fail-closed로 거부합니다.
- `JWT_SECRET`: 세션 서명과 KIS/텔레그램 credential 암호화에 사용하는 서버 비밀값. `NODE_ENV=production`에서 누락되면 credential 암호화가 fail-closed로 중단됩니다.

자동매매 Heartbeat cron은 서비스 규격상 UTC 표현식을 사용합니다. 한국 정규장 09:00~15:30 KST에 맞춰 `0 */5 0-6 * * 1-5`로 등록하고, scheduled handler에서도 KST 장중 여부를 한 번 더 확인합니다.
...
- This app uses a local app-password login instead of Manus OAuth for the owner console.
- `trpc.auth.login` verifies `APP_PASSWORD_HASH`, drops a signed session cookie, and forces a password change while `APP_PASSWORD_MUST_CHANGE=true` or the saved auth state says a change is required.
- Password changes are stored in `APP_PASSWORD_STATE_PATH` (default `.runtime/app-password.json`), so `.runtime/` must remain uncommitted.
...
- Frontend reads auth state with `trpc.auth.me.useQuery()`, renders `LoginGate` when unauthenticated, and invokes `trpc.auth.logout.useMutation()` for logout.
...
- `APP_PASSWORD_HASH`: scrypt password hash generated by `server/_core/appPasswordAuth.ts`
- `APP_PASSWORD_MUST_CHANGE`: set `true` for initial password rotation on first login
- `APP_PASSWORD_STATE_PATH`: local JSON state path for changed password hash
- `APP_LOCAL_OPEN_ID`: local owner identity used in the signed app-password session
- `VITE_APP_ID`: local app/session identifier, defaults to `local-kiss`
- `OWNER_OPEN_ID`, `OWNER_NAME`: optional legacy owner info for OAuth-derived sessions
...
8. Use `useAuth()` for current user state and `LoginGate` for app-password login; avoid direct cookie handling.
